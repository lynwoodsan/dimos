...
void FARUtil::ResetCloudIntensityWithTime(const PointCloudPtr& cloudInOut, const rclcpp::Node::SharedPtr nh) {
  // Standalone: use wall clock instead of ROS clock
  const double now_s = std::chrono::duration<double>(
      std::chrono::steady_clock::now().time_since_epoch()).count();
  const float curTime = static_cast<float>(now_s - FARUtil::systemStartTime);
...
void FARUtil::TransformPCLFrame(const std::string& from_frame_id,
    ...
    // Standalone mode: all data is assumed to be in world frame already.
    // The LCM odometry source (FAST-LIO) outputs in the map frame directly.
    if (from_frame_id != to_frame_id) {
        fprintf(stderr, "[WARN] TransformPCLFrame: frame mismatch (%s -> %s) ignored in standalone mode\n",
...
void FARUtil::TransformPoint3DFrame(const std::string& from_frame_id,
                                    ...
                                    const std::shared_ptr<tf2_ros::Buffer>& /*tf_buffer*/,
                                    Point3D& /*point*/) {
    // Standalone mode: no TF tree available; data is already in world frame.
    if (from_frame_id != to_frame_id) {
        fprintf(stderr, "[WARN] TransformPoint3DFrame: frame mismatch (%s -> %s) ignored in standalone mode\n",
                from_frame_id.c_str(), to_frame_id.c_str());
    }
}
