...
"""
Run script for xArm robot with Claude agent integration.
...
logger = setup_logger("dimos.robot.ufactory.run")

# Load environment variables
load_dotenv()

# System prompt for the xArm manipulation agent
SYSTEM_PROMPT = """You are an intelligent robotic assistant controlling an xArm robot with advanced manipulation capabilities. Your primary role is to help users with pick and place tasks using natural language understanding.

## Your Capabilities:
1. **Visual Perception**: You have access to a ZED stereo camera that provides RGB and depth information
2. **Object Manipulation**: You can pick up and place objects using a 7-DOF robotic arm with a gripper
...
def main():
    """Main entry point."""
    print("\n" + "=" * 60)
    print("xArm Robot with Claude Agent")
    print("=" * 60)
    print("\nThis system integrates:")
    print("  - xArm 7-DOF robot")
    ...
    logger.info("Starting xArm Robot with Agent")

    # Create robot instance
    robot = XArmRobot(arm_ip="10.0.0.197", arm_type="xarm7")
