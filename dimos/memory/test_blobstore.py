...
from dimos.memory.impl.memory import MemoryStore
from dimos.memory.type import _UNLOADED
...
class TestBlobStoreIntegration:
    def test_append_stores_in_blobstore(self, tmp_path: Path) -> None:
        bs = FileBlobStore(tmp_path / "blobs")
        bs.start()
        store = MemoryStore()
    ...
    def test_lazy_data_not_loaded_until_access(self, tmp_path: Path) -> None:
        bs = FileBlobStore(tmp_path / "blobs")
        bs.start()
        store = MemoryStore()
        with store.session(blob_store=bs) as session:
    ...
    def test_lazy_data_loads_correctly(self, tmp_path: Path) -> None:
        bs = FileBlobStore(tmp_path / "blobs")
        bs.start()
        store = MemoryStore()
        with store.session(blob_store=bs) as session:
    ...
    def test_eager_preloads_data(self, tmp_path: Path) -> None:
        bs = FileBlobStore(tmp_path / "blobs")
        bs.start()
        store = MemoryStore()
    ...
    def test_per_stream_eager_override(self, tmp_path: Path) -> None:
        bs = FileBlobStore(tmp_path / "blobs")
        bs.start()
        store = MemoryStore()
        with store.session(blob_store=bs) as session:
    ...
    def test_blobstore_with_vector_search(self, tmp_path: Path) -> None:
        from dimos.memory.vectorstore import MemoryVectorStore
    ...
    def test_blobstore_with_text_search(self, tmp_path: Path) -> None:
        bs = FileBlobStore(tmp_path / "blobs")
        bs.start()
        store = MemoryStore()
        with store.session(blob_store=bs) as session:
    ...
    def test_multiple_appends_get_unique_blobs(self, tmp_path: Path) -> None:
        bs = FileBlobStore(tmp_path / "blobs")
        bs.start()
        store = MemoryStore()
        with store.session(blob_store=bs) as session:
    ...
    def test_fetch_preserves_metadata(self, tmp_path: Path) -> None:
        bs = FileBlobStore(tmp_path / "blobs")
        bs.start()
        store = MemoryStore()
        with store.session(blob_store=bs) as session:
